...
age <- vb_matches %>%
  select(w_p1_age,w_p2_age,l_p1_age,l_p2_age)

age_summary <- age %>%
  summary()

age_diff <- vb_matches %>%
  mutate(diff_age_p1=w_p1_age-l_p1_age,
         diff_age_p2=w_p2_age-l_p2_age)
  
age_diff %>%
  ggplot(aes(diff_age_p1)) +
  geom_boxplot() +
  facet_wrap(~gender)

age_diff %>%
  ggplot(aes(diff_age_p2)) +
  geom_boxplot() +
  facet_wrap(~gender)
