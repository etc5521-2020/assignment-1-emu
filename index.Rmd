...
Looking into Figure \@ref(fig:plot-win), it can be observed that in all the eight years, although team of United States has the highest winning rate at 36 percentage, all the winning rate at home is less than 50%, that is to say, the winning rate at home is not higher than that at away, which shows that the home advantage is not obvious in FVIB competition.
...
Then we comes to women's team. On the basis of the previous part of the method, I added gender screening with screening the teams only for female gender, and calculated the home winning rate, then displayed the results in Figure \@ref(fig:plot-W). It can be seen that with the United States having the highest rate at 39.56%, all of the teams don't have rates over 50%, quite same as the general situation. That indicates that home advantage is still not obvious in women's team.
...
General speaking, home advantage is not tenable for the winning team in FVIB tournament, regardless of gender. However, if we look further, we can find that the winning rate of women at home is higher than that of men in most countries, with the exception of Poland, where the winning rate of men at home is 9.68% higher than that of women's 4.26%. 
...
In Figure \@ref(fig:plot-WR), it can be seen that tournament in Norway has the most proportion of team defeating teams higher than them at about 63.63%, and the lowest tournament is in England at 47.92%. Most of the tournaments have over half teams defeating the higher ranked teams.
...
ggplotly(pWR)
```
Figure \@ref(fig:plot-MR) shows the results of men. The tournament in Greece had about 63.46% teams defeating higher ranked one. The only two that was not over 50% are Brazil and China with proportion at 44.44% and 47.92%. Similar to women, most tournament for men had over half team defeating higher ranked one.
...
In Figure \@ref(fig:plot-RA), the bar chart on the left side is the rate of men with the right-side one showing the rate of women team. It can be observed that the rate of men is about 50.25%, and the rate of women is around 51.15%. We can see that in the AVP competition, the gender difference is not obvious. Similarly, more than half of the teams can beat the higher ranked teams.

```{r plot-RA}
pra <- ggplot(rank_prop_com, aes(x=reorder(gender,rank_prop), y =  rank_prop)) +
  geom_bar(stat="identity",fill = 'purple', color = "pink") +
