...
vb_matches <- vb_matches %>% select(-duration:-l_p2_tot_digs) %>% select(c(-tournament, -match_num, -w_rank, -l_rank)) %>% mutate(w1_birthyear = year(w_p1_birthdate), w2_birthyear = year(w_p2_birthdate), l1_birthyear = year(l_p1_birthdate), l2_birthyear = year(l_p2_birthdate)) 
...
country_fivb <- vb_matches_fivb %>% distinct(w_player1, w_p1_country, w_player2, w_p2_country) %>% group_by(w_p1_country, w_p2_country) %>% 
                tally() %>% arrange(desc(n)) %>% rename(`Player 1 country` = w_p1_country,
                     `Player 2 country` = w_p2_country,
                     `Number of teams` = n)
```
...
p1 <- ggplot(country %>% head(20), aes(`Number of teams`, reorder(`Player 1 country`, +`Number of teams`))) +
  ...
  ylab("Country") +
  ggtitle("Top 20 countries with the most winning teams in FIVB & AVP circuits combined") +
  theme(panel.grid.major = element_blank()) +
  theme(plot.background = element_rect(fill = "grey93"))

ggplotly(p1, tooltip = "x")

p2 <- ggplot(country_fivb %>% head(20), aes(`Number of teams`, reorder(`Player 1 country`, +`Number of teams`))) +
  geom_bar(stat = "identity", fill = "steelblue") +
  xlab("Number of teams") +
  ylab("Country") +
...
vb_matches_FIVB_country <- vb_matches_fivb %>%
  ...
  filter(country == w_p1_country | country == w_p2_country) 
...
Table \@ref(tab:players-win) shows top 10 most successful players in both the volleyball circuits combined. **Juliana Felisberta** leads with 649 combined wins so far followed by **Emanuel Rego** and **Ricardo Santoso**. 
...
p <- ggplot(player_best_plot, aes(x=year, y=count, fill=match_type)) +
    geom_bar(stat='identity', position='dodge') + facet_grid(rows = vars(player)) +
