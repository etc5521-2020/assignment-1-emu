...
first to see how many winners arre from the host country
```{r}
...
vb_matches_FIVB_country1 <- vb_matches_FIVB_country %>%
  filter(country == l_p1_country) 

# count the number of winners who come from the host country
FIVB_country_win <- vb_matches_FIVB_country %>%
  count(w_p1_country) %>%
  setNames(c("country","num_winner"))

# count the number of teams participate in the game
...
FVIB_win_prop = left_join(x = FIVB_country, y = FIVB_country_win, by="country") %>%
  mutate(w_prop = (num_winner/num_team)*100) 
```

```{r}
vb_matches_FIVB_country1 <- vb_matches_FIVB %>%
  select(`country`,`w_p1_country`,`w_p2_country`,`l_p1_country`,`l_p2_country`) %>%
  filter(country == l_p1_country) 

# count the number of winners who come from the host country
FIVB_country_lose <- vb_matches_FIVB_country %>%
  count(l_p1_country) %>%
  setNames(c("country","num_loser"))

#join the data 
FVIB_lose_prop = left_join(x = FIVB_country, y = FIVB_country_lose, by="country") %>%
  mutate(l_prop = (num_loser/num_team)*100) 

FVIB_prop = left_join(x = FVIB_win_prop, y = FVIB_lose_prop, by="country") %>%
  select(-`num_team.y`)
```


```{r}
ggplot(FVIB_win_prop,
       aes(x= country, 
           y = w_prop, group = country, color = country)) +
  geom_point()
```
